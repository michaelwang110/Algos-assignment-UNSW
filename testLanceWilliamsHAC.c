...
void showDendrogram(Dendrogram d)
{
	void doShowDendrogram(Dendrogram); // see later
	doShowDendrogram(d);
}

int main (int argc, char *argv[]) {
    if (argc < 3) {
        printf("Usage: ./testLanceWilliamsHAC [file] [method]\n");
        return EXIT_FAILURE;
    }
    ...
    d->vertex = 0;
    d->left = d->right = NULL;
    d = LanceWilliamsHAC(g, atoi(argv[2]));
    showDendrogram(d);
    freeDendrogram(d);
    return EXIT_SUCCESS;
}
...
void doShowDendrogram(Dendrogram d)
{
	asciinode *proot;
	int xmin, i;
	if (d == NULL) return;
